...
export default function GalleryPage() {
  const [activeCategory, setActiveCategory] = useState<'All' | 'Paintings' | 'Calligraphy' | 'Sketches'>('All');
  ...
  const categories: ('All' | 'Paintings' | 'Calligraphy' | 'Sketches')[] = ['All', 'Paintings', 'Calligraphy', 'Sketches'];
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-16">
              {categories.map((cat) => (
                <button
                  ...
                  className={`font-sans text-xs uppercase tracking-widest px-5 py-2.5 transition-all duration-300 border cursor-pointer ${
                    activeCategory === cat
                      ? 'border-primary bg-primary text-background font-medium'
                      : 'border-primary/10 hover:border-accent text-secondary hover:text-accent bg-transparent'
                  }`}
                >
                  {cat}
                </button>
